...
```{r boxplot centrality by acuity, echo=FALSE}
#Set the dataframe up for a scatter of pt acuity/eigenvector centrality
eigen <- Nodes1019 %>% 
  filter(participant_type == "_PAT") %>% 
  select(Eigenvector_Centrality, Acuity, acuitycat)
#scatterplot of eigenvector centrality by acuity

ggplot(eigen, aes(x = Acuity, y = Eigenvector_Centrality, color = acuitycat)) + geom_boxplot() + labs(title = "Boxplot of patient acuity by eigenvector centrality", x = "Patient Acuity", y = "Eigenvector Centrality")
```
