...
# Now clean the patients data to include: acuity recorded, race black or white, date for which edges are available, chief complaint, MinutesInED, & CC group after re-leveling Chief Complaint.
...
patients35 <- patients_cc %>% filter(Acuity != "Not Recorded", (Race == "Black" | Race == "White" ), 
                                  !is.na(Acuity), d8 %in% Edges_D8s) %>%
  select(sid, d8, Acuity, Race, AGE, Sex, Chief_Complaint, CC_group, MinutesInED, ED_ARRIVAL, ed_departure, Arr_Mode) %>%
  group_by(d8)
...
edges35 <- edges %>% filter(!is.na(i_participant_type), !is.na(j_participant_type), 
  ...
  group_by(D8) %>%
  select(sidi, sidj, i, j, edgeweight, am, pm, D8, idi, idj)
```
```{r Race}
...
ggplot(patients35, aes(Race, d8, color = Race)) +
  geom_point(position = "jitter", shape = 1, size = 2) + scale_x_discrete(drop = FALSE) # (drop = FALSE) will show NA values in the graph
```
...
```{r igraph objects}
net_viz <- graph_from_data_frame(d = edges, vertices = nodes, directed = FALSE)
