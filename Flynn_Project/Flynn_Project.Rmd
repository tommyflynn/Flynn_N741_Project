...
# Chief Complaints
ggplot(patients35, aes(x = Chief_Complaint, y = CC_group, color = CC_group)) +
  geom_count() + theme(legend.position = "none")
nodes %>% filter(participant_type == "_PAT", Acuity != "Not Recorded") %>%
  ggplot(aes(x = LOG_WTDEGREE, y = Acuity)) + geom_point()
# 
# names(nodes)
...
# vertex color
colrs <- c("gray50", "tomato", "gold", "blue", "white")
...
# EDge width
E(shift10)$width <- E(shift10)$weight/6

# Set edge color (optional)
E(shift10)$edge.color <- "gray80"
...
# head(Vshift10)
shift10 <- graph_from_data_frame(d = Eshift1, directed = FALSE, vertices = Vshift1)
colrs <- c("gray50", "tomato", "gold", "blue", "white")
V(shift1)$label <- Eshift1$i
E(shift1)$weight <- Eshift1$edgeweight
V(shift1)$color <- colrs[V(shift1)$label]

plot(shift1, vertex.label.dist = 2, layout = layout_with_lgl)
