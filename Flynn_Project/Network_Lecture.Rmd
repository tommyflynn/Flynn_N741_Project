...
edgeall <- edges %>% 
  mutate(shift = numshift, inum = as.numeric(lut[i_participant_type]), jnum = as.numeric(lut[j_participant_type]), ampm = shiftampm,iname = i_participant_type, jname = j_participant_type)%>%
  select("shift", "ampm", "sidi", "iname", "sidj", "jname", "edgeweight", "D8", "inum", "jnum") %>%
  arrange(shift)
summary(edgeall)
#glimpse(edgeall)
#ties <- matrix(c(edgeall[sidi]))
```
